...
# 
# # change species names to an ID character 
# id_vec <- as.character(seq(1,nrow(data), by = 1))
# data$species <- id_vec
# data$species_space  <- id_vec
# colnames(bird_phylodist2) <- id_vec
# rownames(bird_phylodist2) <- id_vec
# colnames(bird_dist) <- id_vec
# rownames(bird_dist) <- id_vec
# 
# bird_phylodist2[1:10,1:10]
...
m3 <- brm(mine_thrnd  ~ range_logstd.x * Body_mass_log_g + 
          ...
          file = "models/bird_reduced_stan_chain4.rds",
          # control = list(adapt_delta = 0.9),
